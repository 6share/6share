/** \file null_fec.h \brief Compact No-Code FEC
 *
 *  $Author: peltotal $ $Date: 2007/02/26 13:48:19 $ $Revision: 1.19 $
 *
 ...
 *
 * @param tr_block pointer to the source block
 * @param block_len stores the length of block
 * @param es_len encoding symbol length for this block
 *
 * @return pointer to buffer which contains block's data, NULL when memory
 ...
 *
 * @return pointer to buffer which contains object's data, NULL: when memory
 * could not be allocated
 *
 */

char *null_fec_decode_object(trans_obj_t *to, unsigned long long *data_len, alc_session_t *s);
